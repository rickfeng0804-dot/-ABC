  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900/50 rounded-2xl p-5 border border-slate-800 shadow-lg flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <button
          onClick={exportToCsv}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 text-white font-semibold text-xs hover:bg-blue-500 transition shadow-md shadow-blue-900/20 shrink-0"
        >
      ...
      {/* Filter Controls */}
      <div className="bg-slate-900/50 rounded-2xl p-4 border border-slate-800 shadow-lg grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <select
          ...
          className="w-full py-1.5 px-3 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-300 outline-none focus:border-blue-500"
        >
                ...
                filteredTxns.map((txn) => (
                  <tr key={txn.id} className="hover:bg-slate-800/40 transition">
                    ...
                    <td className="py-3.5 px-3">
                      <div className="font-mono font-bold text-blue-400">{txn.toolingId}</div>
                      <div className="font-semibold text-slate-200 line-clamp-1">{txn.toolingName}</div>
                    </td>

                    <td className="py-3.5 px-3">
                      <span className={`px-2.5 py-0.5 rounded text-[11px] font-bold border ${
                        txn.type === '領用' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                        txn.type === '歸還' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                        txn.type === '保養發起' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' :
                    ...
                    <td className="py-3.5 px-3">
                      <div className="font-medium text-slate-200">{txn.operator}</div>
                      <div className="text-slate-500 text-[11px]">{txn.machineOrLine || '-'}</div>
                    </td>

                    <td className="py-3.5 px-3 font-mono">
                      {txn.deltaStrokes ? (
                        <div className="text-emerald-400 font-semibold">+{(txn.deltaStrokes).toLocaleString()} 次</div>
                      ) : (
                        <div className="text-slate-500">-</div>
                      )}
                      <div className="text-slate-300 font-bold">累積: {(txn.resultingStrokes).toLocaleString()} 次</div>
                    </td>

                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-1.5 font-mono text-[11px]">
                        <span className="text-slate-500">{txn.previousStatus}</span>
                        <span className="text-slate-600">→</span>
                        <span className="text-white font-bold">{txn.newStatus}</span>
                      </div>
                    </td>
